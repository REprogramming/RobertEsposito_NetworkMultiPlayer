...
{
	//must be server in order to receive connection
	assert(isServer);
...
void characterCreation(RakNet::Packet* packet)
{
	RakNet::BitStream bs(packet->data, packet->length, false);
	RakNet::MessageID messageId;
	bs.Read(messageId);
...
void OnLobbyReady(RakNet::Packet* packet)
{
	...
	//notify all other connected players that this player has joined the game
...
bool HandleLowLevelPackets(RakNet::Packet* packet)
{
	...
	case ID_INCOMPATIBLE_PROTOCOL_VERSION:
		...
		break;
	case ID_REMOTE_DISCONNECTION_NOTIFICATION: // Server telling the clients of another client disconnecting gracefully.  You can manually broadcast this in a peer to peer environment if you want.
		printf("ID_REMOTE_DISCONNECTION_NOTIFICATION\n");
		break;
	case ID_REMOTE_CONNECTION_LOST: // Server telling the clients of another client disconnecting forcefully.  You can manually broadcast this in a peer to peer environment if you want.
	...
	case ID_NEW_INCOMING_CONNECTION:
		...
		break;
	case ID_REMOTE_NEW_INCOMING_CONNECTION: // Server telling the clients of another client connecting.  You can manually broadcast this in a peer to peer environment if you want.
		OnIncomingConnection(packet);
